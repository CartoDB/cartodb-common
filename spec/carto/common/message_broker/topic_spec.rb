...
RSpec.describe Carto::Common::MessageBroker::Topic do
  ...
  describe '#publish' do
    ...
    it 'adds the token to the attributes when passed' do
      expect(pubsub_topic).to receive(:publish).with("{\"request_id\":\"#{request_id}\"}", { event: 'test_event', token: 'my-secret-token' })
